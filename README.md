...
- **🎨 Media Studio** (⋯ → Media Studio): make a picture, change one, then make it move — in one window, because the work is iterative. An **edit never overwrites its input**, so the next instruction refines the last rather than starting over. Agents can make and change pictures (`/gen/image`, `/gen/image/edit`); **only you can make video**, because a clip costs about $2 and the button says the price next to it
...
- **Skills library** with **Hermes-style auto-learning**: after a completed multi-tool task, a reflection pass decides whether the work distills into a reusable skill — if so it's saved, auto-assigned, and announced in the office. Since v1.0.0 the same reflection can also **correct a skill it already wrote**, and it runs after **failures** too — the strongest evidence a skill is wrong, and previously the one case that was thrown away. It never touches a built-in and never touches one you edited by hand, and the previous version is kept (`POST /registry/skill/revert`)
- **Tools**: per-agent allowlist over the built-in Claude Code tools, plus custom capability via **MCP servers** (name + launch command → injected with `--mcp-config`). Ships with a ready-to-use **🔌 web** server (Playwright MCP) — tick it on an agent and it can **browse, click, type, submit forms and screenshot** real pages (isolated profile, not logged in). See [docs/guide/web-automation.md](docs/guide/web-automation.md). The **🧰 Tools Hub** (⋯ menu) is a browsable catalog of **43 entries** — 15 built-in abilities and **28 MCP servers** added with one click, including **Blender, Godot, Unity, Unreal and Roblox Studio**. The catalog is a file the office fetches **live** (`web/tools.json`), so a renamed package is a PR rather than a release. See [docs/guide/tools-hub.md](docs/guide/tools-hub.md)
...
- **📥 Approvals inbox** *(v1.1)*: one queue for everything that waits on a person — tool permissions, project trust, proposals, an AUTO agent's `STATUS: BLOCKED`, jobs created switched off, and anything a plugin asks via `POST /approvals` — decided in the sidebar, the chat, the terminal or **from your phone** (Telegram buttons; `1 yes` / `2 no note` on any channel). Answering a blocked agent resumes its work with your answer
- **🧪 Skill regression** *(v1.6)*: test cases per skill; the office refuses its own correction when a case breaks — self-improvement that cannot quietly get worse
- **📦 Eight official plugins** *(v1.5–1.6)*: Campaign Board · Content Pipeline · GitHub Triage · Inbox Agent · Weekly Report · Client Folders · Decision Log · Skill Regression — real uses of the engine, one click away, every outward step behind an approval
- **👥 Team templates** *(v1.5)*: a whole team — personas, skills, voices — in one click; `bagidea hire --team dev-shop`
- **📋 A task board and a real calendar** *(v1.4)*: todo / doing / waiting / done · due dates through your rules · dependencies that release themselves · repeat · every delegation, job, action item and workflow run is a card · agents move their own · calendar recurrence, all-day, .ics export/import
- **🧑‍💻 Codex as a system tool** *(v1.4)*: `POST /codex/exec`, `DELEGATE: codex @ project :: task`, a workflow node, `bagidea codex` — sandboxed inside registered projects, diff summarised, cost estimated on the caller; `codex exec review` as a second opinion
- **🧩 Plugin hooks** *(v1.4)*: `onEvent`, `ctx.notify`, `ctx.approvals.ask`, `ctx.tasks` / `ctx.calendar`, `ctx.schedule`, a plugin's own trigger kind and workflow node, and a per-agent memory provider
- **🔀 A workflow engine** *(v1.3)*: the Builder's graph is executed — per node, in parallel where the arrows allow, joins wait, a decision opens one branch, `{{templating}}` carries data — with a persisted run record that survives a restart. Node types: trigger · action (a real agent turn) · fetch · decision · **approval** (waits in the inbox) · **notify** · **delay** · output. **⚡ Triggers** start a workflow without you: schedule, webhook (`POST /hook/<token>`, HMAC-verified), office event, file in a folder, channel keyword
...
bagidea doctor                    why won't it load? port · proxy · firewall · policy
bagidea inbox                     what's waiting for you + what's unread
bagidea approve | deny | answer   decide an inbox item (also from your phone)
bagidea budget [set …]            today's spend vs caps · set office/agent/project caps · digest
...
| `POST /gen/image/edit` `{url, prompt}` | edit an image — always writes a **new** file |
| `GET/POST /gen/video` · `GET /studio` · `GET /studio/list` | 🎨 Media Studio (video is human-UI-only — it costs about $2 a clip) |
| `GET /tools/catalog` | the 🧰 Tools Hub catalog, fetched live with a bundled offline fallback |
| `POST /registry/backend` `{backend}` | 📦 RUN LOCATION — local · docker · ssh (office-wide; per agent in the agent editor) |
| `POST /registry/ghostworktrees` `{enabled}` | 👻 GHOST ISOLATION — a private `git worktree` per ghost |
| `POST /registry/semantic` `{endpoint, model, key?}` | 🔎 SEMANTIC RECALL — embeddings fused with the word index |
| `POST /registry/skill/revert` `{id}` | undo a self-correction, back to the previous version of a learned skill |
...
| `GET /inbox` · `GET /approvals?pending=1` · `POST /approvals` `{title, detail?, options?}` · `POST /approvals/respond` `{id, decision, note?}` | 📥 the approvals queue — every kind that waits on a person, askable by plugins |
| `GET /notify` · `POST /notify/send` `{kind, title, body?}` · `POST /notify/read` · `GET/POST /notify/rules` · `POST /notify/presence` | 🔔 notifications routed by your rules (sidebar · toast · channel · sound · quiet hours · away) |
| `GET /budget` · `POST /budget` `{office:{daily}, agents:{id:{daily}}, projects:{id:{total}}, digest}` · `POST /budget/digest` | 💸 money caps — warn at 80 %, stop new turns at 100 %; estimates labelled ≈ |
| `POST /workflows/run` `{id \| nodes,edges}` · `GET /workflows/run?id=` · `GET /workflows/runs?id=` · `POST /workflows/cancel` | 🔀 the workflow engine — per-node execution, persisted runs, `workflow.run` / `workflow.node` events |
| `GET/POST /triggers` · `POST /triggers/delete` · `POST /triggers/fire` · `POST /hook/<token>` | ⚡ triggers: schedule · webhook (HMAC) · office event · file · channel keyword |
| `GET /tasks` · `GET /tasks/board` · `POST /tasks` · `POST /tasks/update` · `POST /tasks/move` · `POST /tasks/delete` | 📋 the task board — one model behind cards, delegations, jobs, action items and workflow runs |
| `GET/POST /calendar` · `GET /calendar/ics` · `POST /calendar/import` | 📅 events with recurrence and all-day · reminders per occurrence · .ics both ways |
| `GET /codex/status` · `POST /codex/exec` · `POST /codex/review` · `POST /codex/settings` · `POST /codex/cancel` | 🧑‍💻 Codex as a system tool — inside registered projects, sandboxed, budgeted |
| `GET /workflows/types` | node types the office and plugins registered |
| `GET /plugins/library` · `POST /plugins/library/install` | 📦 the official plugins that ship with the office, installed by id |
| `GET /teams` · `POST /teams/hire` | 👥 team templates — a pre-built team hired in one call, never overwriting existing agents |
| `GET /skills/tests?id=` · `POST /skills/tests` · `POST /skills/tests/run` | 🧪 test cases per skill; a self-correction that breaks one is refused |
...
| [Workflow Builder](docs/guide/workflows.md) | a real engine: approval / notify / delay nodes · branches and joins · runs that survive restarts · ⚡ triggers (schedule · webhook · event · file · keyword) |
| [Official plugin library](docs/guide/library.md) | 📦 Campaign Board · Content Pipeline · GitHub Triage · Inbox Agent · Weekly Report · Client Folders · Decision Log · Skill Regression — one click each |
| [Team templates](docs/guide/teams.md) | 👥 dev shop · research lab · content studio · customer support · solo assistant |
| [Tasks & Calendar](docs/guide/tasks.md) | 📋 one board for everything the office does · dependencies · repeat · due-date reminders · 📅 recurrence · .ics both ways |
| [Codex](docs/guide/codex.md) | 🧑‍💻 a colleague you can call in — `DELEGATE: codex`, the workflow node, `bagidea codex`, a second-opinion review |
| [Inbox](docs/guide/inbox.md) | 🔔 notifications with rules · 📥 one approvals queue · answer from your phone · quiet hours |
...
- [x] **📦 Run location** — an agent's work runs in a Docker container or on another
      machine over SSH; a backend that can't be built correctly is refused, not downgraded
- [x] **👻 Ghost isolation** — a private `git worktree` per ghost; parallel work comes
      back as branches and your checkout is never touched
- [x] **🔎 Semantic recall** — any OpenAI-shaped `/embeddings` endpoint (a local Ollama
      keeps memory on the machine); word and meaning rankings fused
- [x] **📚 Skills that correct themselves** — reflection can fix a skill it wrote, and
